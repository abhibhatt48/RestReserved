const axios = require('axios');

// Function to generate the response for managing opening times
function generateManageOpeningTimesResponse(actionType, openingTime, closingTime) {
    // Check if ActionType is provided
    console.log('Checking if ActionType is provided:', actionType);
    if (!actionType) {
        console.log('ActionType not provided. Eliciting slot ActionType.');
        return {
            sessionState: {
                dialogAction: {
                    type: 'ElicitSlot',
                    slotToElicit: 'ActionType', // Elicit the ActionType slot if not provided
                },
                intent: {
                    name: 'ManageOpeningTimes',
                    state: 'InProgress',
                },
            },
            messages: [
                {
                    contentType: 'PlainText',
                    content: 'Do you want to view or update your timings?',
                },
            ],
        };
    }

    
    
    // Construct the payload for the API call
    const payload = {
        email: 'saini7@dal.ca',
        name: 'adda',
        opening_time: openingTime,
        closing_time: closingTime,
        total_tables: 4,
        address: "asdfghjk",
        image_base64:""
    };


    // Make the API call to update the opening times
    console.log('Making API call to update opening times:', payload);
    return axios.post('https://wzdh0w0265.execute-api.us-east-1.amazonaws.com/prod/storeRestaurantDetails', payload)
        .then((response) => {
            console.log('API call response:', response.status, response.data);

            if (response.data === true) {
                // Opening times updated successfully
                const responseMessage = `Opening hours have been ${actionType} to ${openingTime} - ${closingTime}.`;
                console.log('Opening times updated successfully:', responseMessage);
                return {
                    sessionState: {
                        dialogAction: {
                            type: 'Close',
                        },
                        intent: {
                            name: 'ManageOpeningTimes',
                            state: 'Fulfilled',
                        },
                    },
                    messages: [
                        {
                            contentType: 'PlainText',
                            content: responseMessage,
                        },
                    ],
                };
            } else {
                // Opening times update failed
                console.log('Failed to update opening hours. Response data:', response.data);
                return {
                    sessionState: {
                        dialogAction: {
                            type: 'Close',
                        },
                        intent: {
                            name: 'ManageOpeningTimes',
                            state: 'Failed',
                        },
                    },
                    messages: [
                        {
                            contentType: 'PlainText',
                            content: 'Failed to update opening hours. Please try again.',
                        },
                    ],
                };
            }
        })
        .catch((error) => {
            // Handle API call error
            console.error('Error calling API:', error.message);
            return {
                sessionState: {
                    dialogAction: {
                        type: 'Close',
                    },
                    intent: {
                        name: 'ManageOpeningTimes',
                        state: 'Failed',
                    },
                },
                messages: [
                    {
                        contentType: 'PlainText',
                        content: 'Failed to update opening hours. Please try again later.',
                    },
                ],
            };
        });
}

module.exports = async (event) => {
    const intentName = event.sessionState.intent.name;

    if (intentName === 'ManageOpeningTimes') {
        // Extract relevant slots from the resolved slots in the event
        const actionTypeSlot = event.sessionState.intent.slots.ActionType;
        const openingTimeSlot = event.sessionState.intent.slots.OpeningTime;
        const closingTimeSlot = event.sessionState.intent.slots.ClosingTime;

        const actionType = actionTypeSlot && actionTypeSlot.value ? actionTypeSlot.value.originalValue : null;
        let openingTime = openingTimeSlot && openingTimeSlot.value ? openingTimeSlot.value.resolvedValues[0] : null;
        let closingTime = closingTimeSlot && closingTimeSlot.value ? closingTimeSlot.value.resolvedValues[0] : null;

        // Check if openingTime is provided and closingTime is not
        if (openingTime && !closingTime) {
            const [hours, minutes] = openingTime.split(':').map(Number);
            const closingHours = (hours + 9) % 24; // To handle the scenario when the time crosses midnight
            closingTime = `${closingHours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
        }
        
        console.log('Intent is ManageOpeningTimes. Extracted slots:', actionType, openingTime, closingTime);

        // Call the function to handle the response based on the extracted slot values
        return generateManageOpeningTimesResponse(actionType, openingTime, closingTime);
    }

    // Default response if the intent is not ManageOpeningTimes
    console.log('Intent is not ManageOpeningTimes. Returning default response.');
    return {
        statusCode: 200,
        body: JSON.stringify('Hello from Lambda!'),
    };
};


// function generateManageOpeningTimesResponse(actionType, openingTime, closingTime) {
//     const responseMessage = `Opening hours have been ${actionType} to ${openingTime} - ${closingTime}.`;

//     return {
//         sessionState: {
//             dialogAction: {
//                 type: 'Close',
//             },
//             intent: {
//                 name: 'ManageOpeningTimes',
//                 state: 'Fulfilled',
//             },
//         },
//         messages: [
//             {
//                 contentType: 'PlainText',
//                 content: responseMessage,
//             },
//         ],
//     };
// }

// module.exports = async (event) => {
//     const intentName = event.sessionState.intent.name;

//     if (intentName === 'ManageOpeningTimes') {
//         const slots = event.sessionState.intent.slots;
//         return generateManageOpeningTimesResponse(slots.ActionType, slots.OpeningTime, slots.ClosingTime);
//     }

//     return {
//         statusCode: 200,
//         body: JSON.stringify('Hello from Lambda!'),
//     };
// };


// const axios = require('axios');

// function generateManageOpeningTimesResponse(actionType, openingTime, closingTime) {
//     console.log('Checking if ActionType is provided:', actionType);
//     if (!actionType) {
//         console.log('ActionType not provided. Eliciting slot ActionType.');
//         return {
//             sessionState: {
//                 dialogAction: {
//                     type: 'ElicitSlot',
//                     slotToElicit: 'ActionType', 
//                 },
//                 intent: {
//                     name: 'ManageOpeningTimes',
//                     state: 'InProgress',
//                 },
//             },
//             messages: [
//                 {
//                     contentType: 'PlainText',
//                     content: 'Do you want to view or update your timings?',
//                 },
//             ],
//         };
//     }

//     const payload = {
//         email: 'saini7@dal.ca',
//         name: 'adda',
//         opening_time: openingTime,
//         closing_time: closingTime,
//         total_tables: 4,
//         address: "asdfghjk",
//         image_base64:""
//     };

//     console.log('Making API call to update opening times:', payload);
//     return axios.post('https://wzdh0w0265.execute-api.us-east-1.amazonaws.com/prod/storeRestaurantDetails', payload)
//         .then((response) => {
//             console.log('API call response:', response.status, response.data);

//             if (response.data === true) {
//                 const responseMessage = `Opening hours have been ${actionType} to ${openingTime} - ${closingTime}.`;
//                 console.log('Opening times updated successfully:', responseMessage);
//                 return {
//                     sessionState: {
//                         dialogAction: {
//                             type: 'Close',
//                         },
//                         intent: {
//                             name: 'ManageOpeningTimes',
//                             state: 'Fulfilled',
//                         },
//                     },
//                     messages: [
//                         {
//                             contentType: 'PlainText',
//                             content: responseMessage,
//                         },
//                     ],
//                 };
//             } else {
//                 console.log('Failed to update opening hours. Response data:', response.data);
//                 return {
//                     sessionState: {
//                         dialogAction: {
//                             type: 'Close',
//                         },
//                         intent: {
//                             name: 'ManageOpeningTimes',
//                             state: 'Failed',
//                         },
//                     },
//                     messages: [
//                         {
//                             contentType: 'PlainText',
//                             content: 'Failed to update opening hours. Please try again.',
//                         },
//                     ],
//                 };
//             }
//         })
//         .catch((error) => {
//             // Handle API call error
//             console.error('Error calling API:', error.message);
//             return {
//                 sessionState: {
//                     dialogAction: {
//                         type: 'Close',
//                     },
//                     intent: {
//                         name: 'ManageOpeningTimes',
//                         state: 'Failed',
//                     },
//                 },
//                 messages: [
//                     {
//                         contentType: 'PlainText',
//                         content: 'Failed to update opening hours. Please try again later.',
//                     },
//                 ],
//             };
//         });
// }

// module.exports = async (event) => {
//     const intentName = event.sessionState.intent.name;

//     if (intentName === 'ManageOpeningTimes') {
//         const actionTypeSlot = event.sessionState.intent.slots.ActionType;
//         const openingTimeSlot = event.sessionState.intent.slots.OpeningTime;
//         const closingTimeSlot = event.sessionState.intent.slots.ClosingTime;

//         const actionType = actionTypeSlot && actionTypeSlot.value ? actionTypeSlot.value.originalValue : null;
//         let openingTime = openingTimeSlot && openingTimeSlot.value ? openingTimeSlot.value.resolvedValues[0] : null;
//         let closingTime = closingTimeSlot && closingTimeSlot.value ? closingTimeSlot.value.resolvedValues[0] : null;

//         if (openingTime && !closingTime) {
//             const [hours, minutes] = openingTime.split(':').map(Number);
//             const closingHours = (hours + 6) % 24;
//             closingTime = `${closingHours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
//         }
        
//         console.log('Intent is ManageOpeningTimes. Extracted slots:', actionType, openingTime, closingTime);

//         return generateManageOpeningTimesResponse(actionType, openingTime, closingTime);
//     }

//     console.log('Intent is not ManageOpeningTimes. Returning default response.');
//     return {
//         statusCode: 200,
//         body: JSON.stringify('Hello from Lambda!'),
//     };
// };
